...
function TpstOTP() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:3000/api/therapist-verify-otp",{email,otp: Number(enteredOtp),});
  ...
  const handleResend = async () => {
    try {
      const res = await axios.post("http://localhost:3000/api/therapist-resend-otp", { email, });
      setMessage(res.data.message);
      setMessageType("success");
      setTimer(60);
    } catch (err) {
